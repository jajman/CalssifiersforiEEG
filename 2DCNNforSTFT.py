...
try:
    ...
    argVal = 0
save_name='./cnn2-STFT'+str(argVal)+'.txt'
save_path='./cnn2-STFT'+str(argVal)
...
with tf.name_scope("CNNSTFT"):
    num_filters1 = 32

    cx = tf.placeholder(tf.float32, [None, 50,21]) 
    x_image = tf.reshape(cx, [-1, 50, 21, 1])
    ...
    h_pool2 = tf.nn.max_pool(h_conv2_cutoff, ksize=[1, 2, 2, 1],
                             strides=[1, 2, 2, 1], padding='VALID')
    print(h_pool2)
    h_pool2_flat = tf.reshape(h_pool2, [-1, 10 * 3 * num_filters2])

    num_units1 = 10 * 3 * num_filters2
    num_units2 = 512
    num_units3 = 256
    final_output_size = 2
    keep_prob = 0.7
...
'''h2_pooled = sess.run([h_pool2],
                                 feed_dict={cx: s_val[:10], ct: s_ans_val[:10], train_mode: False})
h2_pooled=np.array(h2_pooled)
print(h2_pooled.shape)'''
loss_val, acc_val = sess.run([closs, caccuracy],
                                 feed_dict={cx: s_val[:2000], ct: s_ans_val[:2000], train_mode: False})
